...
export default async function UserPage({
  ...
  try {
    const userLookupResponse = await axios.get(
      `http://localhost:3001/api/userProfile/${username}`
    );
                ...
                <div className="relative z-10">
                  <p className="text-gray-700 leading-relaxed text-lg">
                    ...
                    <span className="font-semibold text-amber-700">
                      {" "}
                      {user.meals_cooked} meals
                    </span>{" "}
                    and created
                    <span className="font-semibold text-amber-700">
                      {" "}
                      {user.created_recipes} recipes
                    </span>{" "}
                    in our community.
                  </p>
                  ...
                  <div className="mt-6 flex items-center">
                    ...
                    <div className="ml-4">
                      <p className="font-medium text-gray-800">
                        Current Rank:{" "}
                        <span className="text-amber-700">{user.rank}</span>
                      </p>
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50">
        <div className="bg-white p-8 rounded-lg shadow-md max-w-md w-full">
          ...
          <p className="text-gray-600">
            Sorry, we couldn't load this user profile. Please try again later.
          </p>
        </div>
      </div>
    );
