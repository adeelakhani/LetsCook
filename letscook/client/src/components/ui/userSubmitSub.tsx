...
export default function UserSubmitSub({ this_user_id, token, postData, submissionData }: UserSubmitSubProps) {
  ...
  const handleApproveSubmission = async () => {
    setIsApproving(true)
    try {
      const response = await axios.post(`${process.env.NEXT_PUBLIC_API_URL}/api/approveSubmission/${submission.id}`, submission, {
        headers: {
  ...
  const handleRejectSubmission = async () => {
    setIsRejecting(true)
    try {
      const response = await axios.patch(`${process.env.NEXT_PUBLIC_API_URL}/api/rejectSubmission/${submission.id}`, {
        headers: {
          "Content-Type": "application/json",
        },
      })
  ...
  const handleDeleteSubmission = async () => {
    if (confirm("Are you sure you want to delete this submission? This action cannot be undone.")) {
      setIsDeleting(true)
      try {
        const response = await axios.delete(`${process.env.NEXT_PUBLIC_API_URL}/api/deleteSubmission/${submission.submitted_by_id}/${submission.post_id}/${submission.id}`, {
          headers: {
            Authorization: `Bearer ${token}`,
            "Content-Type": "application/json",
          },
