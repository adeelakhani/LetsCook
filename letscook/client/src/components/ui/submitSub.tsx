...
export default function SubmitSub({
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const submission = await axios.post(
        `${process.env.NEXT_PUBLIC_API_URL}/api/submitRecipe/${this_user_id}/${post.user_id}/${post.id}/${submissionId}`,
        formData,
        {
          headers: {
  ...
  const handleDeletePost = async () => {
    ...
    try {
      const response = await axios.delete(`${process.env.NEXT_PUBLIC_API_URL}/api/deletePost/${post.user_id}/${post.id}`, {
        headers: {
          Authorization: `Bearer ${token}`,
